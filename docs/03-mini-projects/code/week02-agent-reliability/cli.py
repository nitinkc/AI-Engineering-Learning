...
def _make_idempotency_key(tool_name: str, user_id: str, payload: dict) -> str:
  parts = [tool_name, user_id] + [f"{k}-{v}" for k, v in
...
def cmd_update_ticket(args: argparse.Namespace) -> dict:
  ...
  request = ToolRequest(
      ...
      user_id=args.user_id,
      payload=payload,
      idempotency_key=_make_idempotency_key("update_ticket", args.user_id,
                                            payload),
      risk_level="low",
  )
  return execute_workflow(request, approved=True, fail_once=args.fail_once)
...
def cmd_cancel_subscription(args: argparse.Namespace) -> dict:
  ...
  request = ToolRequest(
      ...
      user_id=args.user_id,
      payload=payload,
      idempotency_key=_make_idempotency_key("cancel_subscription", args.user_id,
                                            payload),
      risk_level="high",
  )
...
def build_parser() -> argparse.ArgumentParser:
  ...
  # update_ticket
  p = subparsers.add_parser("update_ticket",
                            help="Update a support ticket status.")
  _add_common_args(p)
  ...
  # cancel_subscription
  p = subparsers.add_parser("cancel_subscription",
                            help="Cancel a subscription (high-risk).")
  _add_common_args(p)
