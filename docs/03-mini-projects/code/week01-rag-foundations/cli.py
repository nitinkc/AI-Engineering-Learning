...
def build_index(corpus: list[dict]):
    from chunker import Chunk  # local import avoids circular at module level
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # query sub-command
    query_parser = subparsers.add_parser("query", help="Answer a question from the corpus.")
    query_parser.add_argument("question", help="Natural-language question to answer.")
    query_parser.set_defaults(func=cmd_query)

    # summarize sub-command
    summarize_parser = subparsers.add_parser("summarize", help="Summarize a long document.")
