...
2. **Pre-processing** — Your backend sanitises the input, applies safety filters, retrieves relevant
   context (if RAG), and builds the final prompt by assembling system instructions + context + user
   message.
3. **Tokenisation** — The prompt string is converted to token IDs using the model's tokeniser (e.g.,
   `tiktoken` for OpenAI models, `sentencepiece` for many open-source models).
4. **API call** — The token IDs are sent to the model endpoint (OpenAI, Anthropic, or your hosted
   vLLM server).
5. **Model inference** — The Transformer runs a forward pass, producing a probability distribution
   over the vocabulary. It samples (or greedy-decodes) the next token, appends it, and repeats until
   it hits a stop token or the `max_tokens` limit.
6. **De-tokenisation** — Token IDs are converted back to text.
7. **Post-processing** — Your backend validates the output (JSON parse, length check, content
   filter), formats it for the UI, and logs the trace.
