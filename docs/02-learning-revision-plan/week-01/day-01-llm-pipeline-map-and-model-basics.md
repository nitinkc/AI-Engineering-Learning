...
You don't need to memorise every equation in the original ["Attention is All You Need"](https://papers.nips.cc/paper_files/paper/2017/file/3f5ee243547dee91fbd053c1c4a845aa-Paper.pdf) paper, but you do need to explain the mechanism confidently in an interview. 
A Transformer processes text by first breaking it into **tokens** (sub-word fragments), converting each token into a numerical vector,
and then running those vectors through a stack of **attention layers**. Each attention layer lets every token look at every other token and decide how much to "attend" to it — this is how the model understands that "bank" in "river bank" means something different from "bank" in "bank account".
...
2. **Pre-processing** — Your backend sanitises the input, applies safety filters, retrieves relevant context (if RAG), and builds the final prompt by assembling system instructions + context + user message.
3. **Tokenisation** — The prompt string is converted to token IDs using the model's tokeniser (e.g., `tiktoken` for OpenAI models, `sentencepiece` for many open-source models).
4. **API call** — The token IDs are sent to the model endpoint (OpenAI, Anthropic, or your hosted vLLM server).
5. **Model inference** — The Transformer runs a forward pass, producing a probability distribution over the vocabulary. It samples (or greedy-decodes) the next token, appends it, and repeats until it hits a stop token or the `max_tokens` limit.
6. **De-tokenisation** — Token IDs are converted back to text.
7. **Post-processing** — Your backend validates the output (JSON parse, length check, content filter), formats it for the UI, and logs the trace.
8. **Response** — The formatted answer reaches the user.
